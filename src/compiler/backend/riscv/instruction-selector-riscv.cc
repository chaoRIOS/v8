...
void InstructionSelector::VisitWord32AtomicPairLoad(Node* node) {
  ...
  if (projection1) {
    InstructionOperand outputs[] = {g.DefineAsFixed(projection0, a0),
                                    g.DefineAsFixed(projection1, a1)};
    InstructionOperand temps[] = {g.TempRegister(a2)};
    Emit(opcode | AddressingModeField::encode(kMode_MRI), 2, outputs,
         2, inputs, 1, temps);
  } else if (projection0) {
    InstructionOperand outputs[] = {g.DefineAsFixed(projection0, a0)};
    InstructionOperand temps[] = {g.TempRegister(a2), g.TempRegister(a1)};
    Emit(opcode | AddressingModeField::encode(kMode_MRI), 1, outputs,
         2, inputs, 2, temps);
  } else {
    InstructionOperand temps[] = {g.TempRegister(a2), g.TempRegister(a0),
                                  g.TempRegister(a1)};
...
static void VisitPairAtomicBinop(InstructionSelector* selector, Node* node,
  ...
  Node* index = node->InputAt(1);
  Node* value = node->InputAt(2);
  Node* value_high = node->InputAt(node->InputCount()-1);

  InstructionOperand inputs[] = {g.UseRegister(base), g.UseRegister(index),
                                 g.UseFixed(value, a2),
                                 g.UseFixed(value_high, a3)};
  Node* projection0 = NodeProperties::FindProjection(node, 0);
  Node* projection1 = NodeProperties::FindProjection(node, 1);
  if (projection1) {
    InstructionOperand outputs[] = {g.DefineAsFixed(projection0, a0),
                                    g.DefineAsFixed(projection1, a1)};
    InstructionOperand temps[] = {g.TempRegister(a4), g.TempRegister(),
                                  g.TempRegister()};
    ...
    InstructionOperand outputs[] = {g.DefineAsFixed(projection0, a0)};
    InstructionOperand temps[] = {g.TempRegister(a4), g.TempRegister(a1),
                                  g.TempRegister()};
    selector->Emit(opcode | AddressingModeField::encode(kMode_None),
                   1, outputs, 4, inputs,
                   3, temps);
  } else {
    InstructionOperand temps[] = {g.TempRegister(a4), g.TempRegister(a0),
                                  g.TempRegister(a1)};
    selector->Emit(opcode | AddressingModeField::encode(kMode_None),
                   0, nullptr, 4, inputs,
                   3, temps);
...
void InstructionSelector::VisitWord32AtomicPairCompareExchange(Node* node) {
  ...
  InstructionOperand inputs[] = {
      g.UseRegister(node->InputAt(0)),  g.UseRegister(node->InputAt(1)),
      g.UseFixed(node->InputAt(2), a3), g.UseFixed(node->InputAt(3), a4),
      g.UseFixed(node->InputAt(4), a5), g.UseUniqueRegister(node->InputAt(5))};
  ...
  if (projection1) {
    InstructionOperand outputs[] = {g.DefineAsFixed(projection0, a0),
                                    g.DefineAsFixed(projection1, a1)};
    InstructionOperand temps[] = {g.TempRegister(a2)};
    Emit(code, 2, outputs, arraysize(inputs), inputs,
         1, temps);
  } else if (projection0) {
    InstructionOperand outputs[] = {g.DefineAsFixed(projection0, a0)};
    InstructionOperand temps[] = {g.TempRegister(a2), g.TempRegister(a1)};
    ...
    InstructionOperand temps[] = {g.TempRegister(a2), g.TempRegister(a0),
                                  g.TempRegister(a1)};
    Emit(code, 0, nullptr, arraysize(inputs), inputs, 3, temps);
  }
}
